...
public class previous : MonoBehaviour {
	...
	public LayerMask previousMask; // Mask to filter out what is and is not the previous button
	...
	private bool shouldClick; // True if the laser hits the previous button
	...
	void Update()
	{
		...
		if (touchpad.y < 0.0f && touchpad.x > -0.5f && touchpad.x < 0.5f && Controller.GetPress (SteamVR_Controller.ButtonMask.Touchpad)) {
			...
			if (Physics.Raycast (trackedObj.transform.position, transform.forward, out hit, 100, previousMask)) {
				hitPoint = hit.point;
	...
	//this function makes clicking the box activate the gallery script, and flip to the previous image

	void buttonPress()
	{
		//this increments the variable num.  Num is the number in the array of images, so this goes to the previous image
